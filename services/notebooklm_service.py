...
### 절대 작성 규칙
1. 모든 문장은 반드시 '~함.', '~임.'과 같이 **명사형 또는 종결형 종결 어미**로 끝낼 것. 절대 '~입니다', '~한다' 등을 사용하지 마시오. 기본 언어는 **한국어**이며, 고유명사나 전문용어 등은 영어를 사용할 경우 **영문 그대로** 표기할 것.
...
3. 본문은 평가자가 한눈에 파악할 수 있도록 직관성과 전문성을 바탕으로 구조화하되, 기계적인 분할을 피하고 **내용의 복잡도, 중요도, 논리적 위계에 따라 유연하게 조절하여 인간이 작성한 것처럼 자연스럽게 구성**할 것.
    ...
    - **[순번 체계 예외] 단, 문맥상 순서나 넘버링(①, ② 등)으로 명확한 구분이 필요한 경우, 기계적인 글머리기호 대신 마커([L1] 등) 바로 뒤에 해당 순번 기호를 직접 결합하여 작성할 것.** 이 넘버링은 [L1], [L2], [L3]를 비롯한 모든 단계에 포함될 수 있음.
        - [사용 마커 및 마크다운 형태 예시]
            1단계: * [L1] 
            2단계:   * [L2]
            3단계:     * [L3]
...
8. 표(Table) 작성 요청 시, 제공된 열(Column) 구조를 유지하되 **정보량에 따라 행(Row)은 자유롭게 추가**할 것.
    - 표 내부 데이터 작성 시 마커([L1], [L2] 등) 사용 규칙:
        - 단순 수치, 단위, 단답형 단어, 짧은 명사구 앞에는 **마커를 절대 사용하지 말 것**. (예: 2.5, 15%, 미국 / 구글)
        - 표 내부에서 **여러 줄의 서술형 문장을 작성**하거나, **명확한 상하위 리스트(위계) 구분이 반드시 필요한 경우**에 한해서만 예외적으로 마커를 사용할 것.
...
### [작성 규칙 및 제약 조건]
1. **문체 준수**: 모든 문장은 반드시 '~함.', '~임.'과 같은 명사형/종결형 어미로 끝내십시오. 기본 언어는 **한국어**이며, 고유명사나 전문용어 등은 영어를 사용할 경우 **영문 그대로** 표기할 것.
